...
function labelPkg(p: string): string {
  return { exterior: "Exterior", interior: "Interior", full: "Full Wash", wax: "Wax" }[p] ?? p;
}
...
async function computeSummary(
  ...
  snap.docs.forEach((d) => {
    ...
    byPackage[v.package] = (byPackage[v.package] ?? 0) + 1;
  });
...
function buildEmailHtml(summary: DaySummary, dateLabel: string): string {
  const typeRows = Object.entries(summary.byType)
    .map(([k, v]) => `<tr><td style="padding:6px 12px;">${labelType(k)}</td><td style="padding:6px 12px;text-align:right;font-weight:bold;">${v}</td></tr>`)
    .join("");

  const pkgRows = Object.entries(summary.byPackage)
    .map(([k, v]) => `<tr><td style="padding:6px 12px;">${labelPkg(k)}</td><td style="padding:6px 12px;text-align:right;font-weight:bold;">${v}</td></tr>`)
    .join("");
  ...
  <div style="max-width:520px;margin:32px auto;background:#161A22;border-radius:16px;overflow:hidden;border:1px solid #252B38;">
    ...
    <div style="padding:28px;">
      <div style="display:flex;gap:16px;margin-bottom:24px;">
        ...
        ${summary.unpaid > 0 ? `<div style="flex:1;background:#0D0F14;border-radius:10px;padding:16px;border:1px solid #252B38;">
...
async function sendDayEmail(dayStart: Date, dayEnd: Date): Promise<void> {
  ...
  if (!ownerEmail) {
    functions.logger.warn("No owner email configured — skipping day email.");
    return;
  }
  ...
  // Credentials from Firebase Functions config (set via firebase functions:config:set)
  const smtpUser: string = functions.config().smtp?.user ?? process.env.SMTP_USER ?? "";
  const smtpPass: string = functions.config().smtp?.pass ?? process.env.SMTP_PASS ?? "";
...
export const scheduledDayClose = functions.pubsub
  ...
  .onRun(async () => {
    ...
    await sendDayEmail(istMidnight, now);
  });
